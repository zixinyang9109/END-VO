...
//        kps_right.push_back(cv::Point2f(px[0], px[1]));
    ...
    imshow("left",image_left_resized);
    imshow("right",image_right_resized);
    imshow("depth",depth_resized/200);

    Mat diff;
    absdiff(img1_warp,image_left_resized,diff);
    imshow("diff",diff);
    minmax(diff,"diff photometric");
    waitKey(0);

    Mat mask = Mat::ones(Size(cols,rows) , CV_8UC1);

//    Scalar neam = cv::mean(diff,mask);
//    printf("average difference mat %d \n",neam.val[0]);
    ...
    for (int u=0; u<cols;u++){
        for (int v=0;v<rows;v++){
            auto value = diff.ptr<uchar>(v)[u];
            if (value>0){
                mean_mat = mean_mat + value;
                num_val++;
            }
        }
    }
    ...
    for (int i =0;i<pt1.size();i++){
        if (status[i]){
            ...
            auto pixel_value_left = GetPixelValue(image_left_resized,u,v);
            auto pixel_value_right = GetPixelValue(image_right_resized,u_right,v_right);
            auto diff_value = abs(pixel_value_left-pixel_value_right);

            photo_diff.push_back(diff_value);

        }
    }

    double sum = accumulate(begin(photo_diff),end(photo_diff),0.0);
    double mean = sum/photo_diff.size();
    printf("the mean value from KLT: %f",mean);
...
//    double depth_pixel = depth.at<uchar>(u,v);
//    auto pword = camera_left_->pixel2world(pixel,camera_left_->pose(),depth_pixel);
//    auto pixel_right = camera_right_->world2pixel(pword,camera_right_->pose());
//    cout<<" left   "<<pixel<<endl;
//    cout<<" right  "<<pixel_right<<endl;
