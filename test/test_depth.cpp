...
bool getcamera(std::vector<myslam::Camera::Ptr> &cameras_) {
    // read camera intrinsics and extrinsics
    auto resize_scale = myslam::Config::Get<double> ("resize_scale");
...
int main(){
    ...
    string color_path = "/media/yzx/Elements/SCARED_related/SCARED_RECONSTRUCTION_rectify_black/Dataset1/keyframe_2/data/left/00000.jpg";
    ...
    cv::resize(img1_gray, image_left_resized, cv::Size(), resize_scale, resize_scale,
               cv::INTER_NEAREST);
    cv::resize(img2_gray, image_right_resized, cv::Size(), resize_scale, resize_scale,
               cv::INTER_NEAREST);
    cv::resize(depth, depth_resized, cv::Size(), resize_scale, resize_scale,
               cv::INTER_NEAREST);
    cv::resize(color, color, cv::Size(), resize_scale, resize_scale,
               cv::INTER_NEAREST);
    ...
    v_min = v_min*resize_scale;
    v_max = v_max*resize_scale;
    u_min = u_min*resize_scale;
    u_max = u_max*resize_scale;
...
//    cv::waitKey(0);
    ...
    Ptr<GFTTDetector> detector = GFTTDetector::create(5000, 0.01, 20); // maximum 500 keypoints
    ...
    for (size_t i = 0; i < current_frame_->features_left_.size(); ++i) {
        ...
        if (myslam::triangulation(poses, points, pworld) && pworld[2] > 0) {
            ...
            cout<< "pred depth "<<d/200<<endl;
            cout<<"diff "<<depth_tri-d/200<<endl;
            if(abs(depth_tri-d/200)<5){
